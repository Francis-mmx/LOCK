...
void pc0_rang_limit(void *low_addr, void *high_addr)
{
    ...
    DSP_PC_LIMH = (u32)high_addr;
    DSP_PC_LIML = (u32)low_addr;
}
/* --------------------------------------------------------------------------*/
/**
 * @brief pc0_rang_limit cpu0 pcÖžÕëÔËÐÐ·¶Î§ÏÞÖÆ
...
//           (u32)high_addr);
    ...
    else
    {
        ...
        DSPCON &= ~((1 << limit_index) << 8);
        DSPCON |= (1 << limit_index) << 24;
    }

    debug_exit_critical();

#if 0
    printf("WR_LIM%dL:%x--WR_LIM%dH:%x--DSPCON:%x--PRP_ALLOW_NUM0:%x--%x\n",
           limit_index,
...
//    printf("cpu0 ex limit low_addr:%x--high_addr:%x\n", (u32)low_addr, (u32)high_addr);
    ...
    wr_limit[limit_index].wr_limit_h = (u32)high_addr;
    wr_limit[limit_index].wr_limit_l = (u32)low_addr;
    DSP_BF_CON |= BIT(16 + limit_index);
    DSPCON |= BIT(16);

#if 0
    printf("WR_LIM%dL:%x--WR_LIM%dH:%x---DSPCON:%x\n",
           limit_index,
           wr_limit[limit_index].wr_limit_l,
           limit_index,
           wr_limit[limit_index].wr_limit_h,
...
u32 cpu_write_range_unlimit(void *low_addr)
{
    char i;
    debug_enter_critical();
    for(i = 0; i < 8; i++)
...
//  DBG_CON1 |= 3;  ///< enable peripheral_bus_inv_en bus_inv_expt_en
    ...
    debug_index = 0;
    printf("DSPCON : 0x%x \n", DSPCON);
    DSPCON &= ~(BIT(16) | BIT(24) | BIT(25) | BIT(26) | BIT(27) | BIT(28) | BIT(29));
