...
u8 device_status[10] = {0};//开机设备状态
u8 auto_check_status[10] = {0};//自检设备状态
u8 auto_check_flag = 0;//自检标志位
u8 on_homepage = 0;//在主页标识
extern u8 sys_lock_time;//系统锁定时间
...
/*****************************校验位 ************************************/
u16 calculate_checksum(void *buf, u16 data_length, u8 flag) //0--计算串口数据 1--计算记录数据 2--计算串口接收数据
{
    u16 sum = 0;
    u8 sum_H, sum_L, i;
    if(flag == 0)
...
//    printf("buffer_data check %d name %s\n",buffer_data.check,buffer_data.name);
    ...
    else                                                        //写满500条后，开始写缓冲扇区，但是还未写满 (start_pointer-MAX_RECORD_NUM                     0-start_pointer)
    {
        ...
        if(start_pointer >= RECORDS_PER_SECTOR)//起始指针超过了一个扇区的记录数，从缓冲扇区拷贝到了普通扇区中
        {
            for(int i = 0; i < (start_pointer / RECORDS_PER_SECTOR * RECORDS_PER_SECTOR); i++)
            {
                len = dev_bulk_read(dev, &read_data, RECORD_BASE_ADDR - RECORD_SIZE * i - RECORD_SIZE, RECORD_SIZE);  //从拷贝后的扇区地址中读出数据
                if(len != RECORD_SIZE)
                {
                    printf("read error!\n");
                }
...
u8 erase_flash(u8 num)
{
    ...
    if(num == 0xFF)
    {
        ...
        list_num_increase = 1;
        memset(time_idx, 0, sizeof(time_idx));
        memset(list_valid_array, 0, sizeof(list_valid_array));
...
u16 visit_idx = 0;//指示查找访问记录在索引表中所处的位置
u16 pre_page_cnt = 0;//按一次 上一页 按钮后，匹配的访问记录数量

u16 next_page_cnt = 0;//按一次 下一页 按钮后，匹配的访问记录数量
u32 normal_idx = MAX_RECORD_NUM;//指示在普通扇区查找时的索引
u16 copy_buffer_idx = 0;//指示在拷贝后的扇区查找时的索引
/************************************************************查找访问记录************************************************************/
u16 match_visit_time(struct sys_time *sys_time, user_visit *visit, u16 idx, u8 flag) //0--上一页  1--下一页  idx是time_idx数组下标，代表翻页进度
{
    ...
    if(checksum != read_data.check || count_buffer == 0)//判断条件，缓冲扇区还未存入数据
    {
        if(start_pointer == 0)
        {
            if(flag)                                             //下一页
            {
                ...
                for(i = visit_idx; i < time_idx_count; i--)
                {
                    ...
                    visit[count] = read_data;//把符合条件的记录存入数组，返回出去调用
                    count++;
            ...
            else                                                //上一页
            {
                for(i = visit_idx; i < time_idx_count ; i++)
                {
                    ...
                    if(time_idx[i].year == time_idx[i + 1].year && time_idx[i].month == time_idx[i + 1].month && time_idx[i].day == time_idx[i + 1].day)
                    {
                        continue;//时间相同代表是同一天的访问记录，继续循环查找
                    }
                    else
                    {
                        break;//时间不同则跳出循环，查找完成
                    }
                }
            }
        }
        else
        {
            ...
            if(flag)
            {
                for(i = visit_idx; i < time_idx_count; i--)
                {
                    read_idx = (start_pointer + i) % MAX_RECORD_NUM;//查找顺序是从start到第500条记录，然后从0到start
    ...
    else                    //缓冲扇区有数据，可能是记录刚刚超过500，直接存放在缓冲扇区，也可能是远超500，已经拷贝到普通扇区后，又重新存放在缓冲扇区
    {
        /*************************第二种情况：超过MAX_RECORD_NUM，缓冲扇区未满128，startpointer 在第一个扇区*************************/
        if(flag)        //下一页
        {
            for(i = visit_idx; i < time_idx_count; i--)
            {
                ...
                {
                    /*************************第四种情况：超过500，缓冲扇区有记录，未满128，startpointer在扇区中间某位置*************************/
                    ...
                    copy_buffer_idx--;
                    if(start_pointer / RECORDS_PER_SECTOR && copy_buffer_idx < MAX_RECORD_NUM)//读拷贝扇区       通过start_pointer的值判断拷贝到哪一个扇区了
                    {
...
void last_show_device()
{
    if((device_status[0] + device_status[1] + device_status[2] + device_status[3] + device_status[4] + device_status[5] +
            device_status[6] + device_status[7] + device_status[8] + device_status[9]) <= 10)//还没有显示过设备状态
    {
        ...
        device_status[5] = !get_tp_init_state_func();//获取tp状态
...
//正常上电初始化后不显示ui入口，等待接收设备状态后延时5s再显示，或者未收到设备状态等待35s全部显示异常，再进入ui入口
...
void record_pre_page()
{
    ...
    else
    {
        ...
        get_system_time(&sys_time);
        record_tmp = visit_idx + pre_page_cnt;
        if(record_tmp < time_idx_count && visit_idx != (time_idx_count - 1))
        {
            record_now_page = 0;
...
void record_next_page()
{
    ...
    else
    {
        ...
        record_tmp = visit_idx - next_page_cnt;
        if(record_tmp < MAX_RECORD_NUM)
        {
            ...
            do
            {
                ...
                printf("next_page_cnt %d visit_idx %d normal_idx %d\n", visit_global_count, visit_idx, normal_idx);

            }
            while(visit_global_count == 0 && visit_idx > MAX_RECORD_NUM);
...
void factory_reset_progressbar()
{
    ...
    {
        persent = 0;
        ui_pic_show_image_by_id(ENC_LAY_BACK_PIC, 0);
