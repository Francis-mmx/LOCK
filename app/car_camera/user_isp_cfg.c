...
//将场景添加到scane_type数组中
const unsigned char scane_type[] = {SCENE_NIGHT, SCENE_INDOOR, SCENE_OUTCLUDY, SCENE_OUTDOOR};

//添加isp效果对应段的阈值范围
#define USER_ISP_CFG_NIGHT_L    ((0 << 8))
#define USER_ISP_CFG_NIGHT_H    ((0 << 8)  + 30)
#define USER_ISP_CFG_INDOOR_L   ((0 << 8)  + 50)
...
/* #define USER_ISP_CFG_NIGHT_L    ((0 << 8)) */
/* #define USER_ISP_CFG_NIGHT_H    ((5 << 8)    - 128) */

/* #define USER_ISP_CFG_INDOOR_L   ((5 << 8)    + 128) */
/* #define USER_ISP_CFG_INDOOR_H   ((9 << 8)    - 128) */

/* #define USER_ISP_CFG_OUTCLUDY_L ((9 << 8)    + 128) */
/* #define USER_ISP_CFG_OUTCLUDY_H ((16 << 8)   - 128) */

/* #define USER_ISP_CFG_OUTDOOR_L  ((16 << 8)   + 128) */
/* #define USER_ISP_CFG_OUTDOOR_H  ((20 << 8)  + 128) */

//将isp效果阈值范围添加到isp_par数组,按照场景的顺序添加
const struct usr_isp_cfg_par isp_par[] =
{
...
const unsigned char wdr_en[] = {1, 0, 0, 0};/* 0:该场景不开wdr(无论菜单是否打开wdr使能，该场景都不开wdr)
...
int user_isp_cfg_init(void)
{
    FILE *fp[MAX_ISP_SCENES];
    int size;
    int i;

    puts("user_isp_cfg_init");
    memset(__this->scenes, 0x0, sizeof(__this->scenes));

    for(i = 0; i < MAX_ISP_SCENES; i++)
    {
        fp[i] = fopen(isp_cfg_file[i], "r");
    }
    ...
    for(i = 0; i < MAX_ISP_SCENES; i++)
    {
        ...
        if(__this->scenes[i].buf)
        {
            ...
            fread(fp[i], __this->scenes[i].buf, size);
        }
        fclose(fp[i]);
    }
...
/* late_initcall(user_isp_cfg_init); */
