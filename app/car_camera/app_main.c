...
#define ANSWER_TAG 0x21             //应答标识符

u32 spin_lock_cnt[2] = {0};

u32 uart_timer_handle[10] = {0};    //定时器句柄
u8 ani_flag = 0;                    //1--动画或进度条
u8 sys_lock_time;                   //系统锁定时间
u8 answer_flag = 1;                 //0--未收到回复 1--收到回复
u8 lock_on = 1;                     //判断是否在主页或者密码页
u8 recv_buffer[BUFFER_SIZE];        // 接收缓冲区
u8 write_index = 0;                 // 写指针
u8 read_index = 0;                  // 读指针
u8 data_count = 0;                  //缓冲区内数量

extern u8 device_status[10];        //设备状态
extern u8 auto_check_status[10];    //自检设备状态
extern u8 auto_check_flag;          //自检标志位
extern u8 on_homepage;              //判断是否在主页
extern u8 tx_flag;                  //重发标志
extern u8 power_flag;               //权限标志
...
int uart_send_package(u8 command, u8 *data, u8 com_len)
{
    ...
    //start_app(&uart_buf);
    sys_timeout_add(&uart_buf, start_app, 100);             //延时启动重发，否则可能出现重发连续，响应多次
    return 0;
}
        ...
        if(tx_flag < MAX_TRANSMIT)
        {
            ...
            switch(com)
            {
                case 0xA0:
                    ...
                    break;
                case 0xA1:
                    uart_timer_handle[1] = sys_timeout_add(&uart_buf, transmit_callback, 100); 
                    break;
                case 0xA2:
                    uart_timer_handle[2] = sys_timeout_add(&uart_buf, transmit_callback, 100); 
                    break;
                case 0xA3:
                    uart_timer_handle[3] = sys_timeout_add(&uart_buf, transmit_callback, 100); 
                    break;
                case 0xA4:
                    uart_timer_handle[4] = sys_timeout_add(&uart_buf, transmit_callback, 100); 
                    break;
                case 0x10:
                    uart_timer_handle[5] = sys_timeout_add(&uart_buf, transmit_callback, 100); 
                    break;
                case 0x11:
                    uart_timer_handle[6] = sys_timeout_add(&uart_buf, transmit_callback, 100); 
                    break;
            }
        }
        else
    ...
    if(lock_on)
    {
        ...
        if(user_data != NULL)
        {
            ...
            for(int i = 0; i < user_data->key_num; i++)
            {
                ...
                {
                    now_user.unlock.unlock_mode = buf[7];//判断解锁方式
                    break;
                }
    ...
    if(buf[2] == 0x11)                                  //取消重发时，锁->屏的标识符为0x11
    {
        ...
        else if(buf[4] == 0xA4)
        {
            if(auto_check_flag)
            {
                ...
                auto_check_status[5] = !get_tp_init_state_func();//获取tp状态
...
void get_system_lock_status(u8 *buf)
{
    ...
    else if(buf[5] == 0x02)//提示系统锁定
    {
        command_buf = voice;
        data_buf = system_locked;
        uart_send_package(command_buf, &data_buf, 1);
    }
    else if(buf[5] == 0)//退出系统锁定
    {
        for(u16 i = 0; i < sys_lock_time + 1; i++)
        {
...
int uart_receive_package(u8 *buf, int len)  //串口接收   最大接收长度512字节  接收时间580ms左右
{
    if((len == (buf[3] + 6)) && buf[0] == 0xAA && buf[1] == 0xBB)
    {
        for(u8 i = 0; i < len; i++)
        {
            recv_buffer[write_index] = buf[i];                  //接收到的数据存入环形缓冲区
