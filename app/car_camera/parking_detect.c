...
#define UVC_PARKCMD_ERR_DEBOUNCE  1
...
static int parking_detect_init()
{
    ...
    if(!__this->timer)
    {
        __this->timer =  sys_timer_add(NULL, parking_detect_ctrl, DETECT_TIME);//100ms
    }
    /* #elif (defined CONFIG_VIDEO3_ENABLE) */
#elif (CONFIG_VIDEO_PARK_DECT == 3)
    __this->uvc_id = -1;
...
//关闭后拉拍照命令功能
static int uvc_event_handler(void *priv, enum uvc_event event)
{
    return 0;
}
